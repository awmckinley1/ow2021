...
%	\draw[blue] ({0},{0}) arc (-40:40:0.3);
	...
	\draw[blue,densely dotted] (-0.5,-{(\slitsep+\slitwidth)/2}) -- (3,-{(\slitsep+\slitwidth)/2});
	\draw[blue,<->] (-1.3,-{(\slitsep+\slitwidth)/2}) -- node[left]{$a$} (-1.3,{(\slitsep+\slitwidth)/2});
	\draw[black,<->,thick] (-0.7,0) -- node[left]{$x$} (-0.7,{(\slitsep)/2});
	\draw[black,<->,thick] (-0.7,0) -- node[left]{$x$} (-0.7,{-(\slitsep)/2});
	\draw[black,|<->|,thick] (-0,{((\slitsep)/2)-\element}) -- node[left]{$\Delta x$} (-0,{((\slitsep)/2)+\element});
		\draw[black,|<->|,thick] (-0,{-((\slitsep)/2)-\element}) -- node[left]{$\Delta x$} (-0,{-((\slitsep)/2)+\element});
...
\filldraw[blue] ({\distancetoscreen},{\pointP}) circle(2pt) node[above right]{$P$};
\draw[red,->](0,{\slitsep/2}) -- ({\distancetoscreen},{\pointP});
\draw[red,->](0,-{\slitsep/2}) -- ({\distancetoscreen},{\pointP});
\draw[red,densely dotted](0,0) -- ({\distancetoscreen},{\pointP});

\draw[blue,<->,thick] ({\distancetoscreen+0.5},0) -- node[right]{$X$} ({\distancetoscreen+0.5},{\pointP});
\draw[<->] (0:0.5*\distancetoscreen) arc  (0:11.5:0.5*\distancetoscreen) node[midway,right]{ $\theta$} ;
