...
%    ]
%% Incident wave:
	\newcommand \heightofwall{3}
	\newcommand \slitsep{1.5}
	\newcommand \slitwidth{0.0}
	\newcommand \wallwidth{0.2}
	\newcommand \waveheight{1.9}
...
\foreach \w in {0,...,6}
{
	\draw[red](-{(\waveoffset+\w*\wavespace)},-\waveheight) -- (-{(\waveoffset+\w*\wavespace)},+\waveheight);
}

\foreach \a in {1,...,7}
{
\draw[blue] ({\a*\wavespace*cos(-60)},{(1/2)-\a*\wavespace*sin(60)}) arc (-60:60:{\a*\wavespace});
}

\foreach \a in {1,...,7}
{
\draw[red] ({\a*\wavespace*cos(-60)},-{(1/2)-\a*\wavespace*sin(60)}) arc (-60:60:{\a*\wavespace});
}

\foreach \a in {1,...,7}
{
\draw[black] ({\a*\wavespace*cos(-60)},{(0)-\a*\wavespace*sin(60)}) arc (-60:60:{\a*\wavespace});
}

\foreach \b in {-30,0,30}
{
\draw[->,black,thick] (0, 0) -- ++ (\b:3);
}

\foreach \b in {-30,0,30}
{
\draw[->,red,thick] (0, -0.5) -- ++ (\b:3);
}

\foreach \b in {-30,0,30}
{
\draw[->,blue,thick] (0, 0.5) -- ++ (\b:3);
}
