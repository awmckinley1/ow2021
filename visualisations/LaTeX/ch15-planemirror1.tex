...
%\newcommand \pointAx{
    ...
    % media
    \fill[gray!25!,opacity=.05] (\leftlimit,0) rectangle (\rightlimit,\toplimit);
    \fill[gray!60!,opacity=.3] (\leftlimit,0) rectangle (\rightlimit,\bottomlimit);
...
%    \node[] at ({0.8*\rightlimit},{\interfaceheight-0.5}) {$n_2$};
    ...
    \draw[red,ultra thick,reverse directed,dashed] (O) -- ({270-\angleofincidence}:{2*\toplimit});
...
% \node[above right] at ({270+asin(\refractivenA*sin(\angleofincidence)/\refractivenB)}:{-0.9*\bottomlimit/cos(asin(\refractivenA*sin(\angleofincidence)/\refractivenB))})  {$B$};
    ...
    \draw[fill] ({90+\angleofincidence}:{2*\toplimit}) circle (0.05 cm) node[left] {$P$};
    \draw[fill] ({270-\angleofincidence}:{2*\toplimit}) circle (0.05 cm) node[left] {$P'$};
    \draw[fill] ({90-\angleofincidence}:{2*\toplimit}) circle (0.05 cm) node[right] {Observer};
